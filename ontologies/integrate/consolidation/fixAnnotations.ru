...
# This is not currently used as it is deamed too strict:
# it overwrites local annotations in favor of the source version.
...
WHERE {
    ...
    FILTER(STR(?targetAnnotation) != STR(?sourceAnnotation))
};

INSERT {
    GRAPH ?ontology {
        ?term ?annotationProperty ?sourceAnnotationWithLang
    }
}
WHERE {
    ?targetOntology a owl:Ontology;
